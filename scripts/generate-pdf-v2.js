  ...
  try {
    ...
    switch (command) {
      case "list":
        ...
        if (allEditions.length === 0) {
          ...
          allEditions.forEach((edition) => {
            ...
            console.log(
              `- Issue ${edition.issueNumber} (ID: ${edition.id}) - ${publishedDate}`
            );
          });
        }
        break;
      ...
      case "generate":
        ...
        console.log(
          `Print URL: ${BASE_URL}/print?issue=${edition.issueNumber}`
        );

        await generator.generateFromIssue(
          edition.issueNumber,
          outputPath,
          BASE_URL
        );
      ...
      case "preview":
        ...
        await generator.generatePreviewScreenshot(
          previewUrl,
          previewOutputPath
        );
      ...
      case "info":
        ...
        const infoEdition = await db.query.editions.findFirst({
          ...
          with: {
            ...
            poems: {
              ...
              orderBy: (editionPoems, { asc }) => [
                asc(editionPoems.orderPosition),
              ],
            },
            images: {
              with: {
              ...
              orderBy: (editionImages, { asc }) => [
                asc(editionImages.orderPosition),
              ],
            },
          },
        });
        ...
        console.log(
          `Edition ${infoEdition.id} - Issue ${infoEdition.issueNumber}`
        );
        console.log(
          `Published: ${new Date(infoEdition.publishedAt).toLocaleDateString()}`
        );
        console.log(
          `Cover Image: ${
            infoEdition.coverImage ? infoEdition.coverImage.filename : "None"
          }`
        );
        console.log(
          `Print URL: ${BASE_URL}/print?issue=${infoEdition.issueNumber}`
        );
        console.log("");
        console.log("Content:");
        ...
        infoEdition.articles?.forEach((editionArticle, index) => {
          ...
          const authors =
            article.authors?.map((aa) => aa.author.name).join(", ") || "";
          console.log(
            `    ${index + 1}. ${article.hed}${
              authors ? ` (by ${authors})` : ""
            }`
        ...
        infoEdition.poems?.forEach((editionPoem, index) => {
          ...
          const authors =
            poem.authors?.map((pa) => pa.author.name).join(", ") || "";
          console.log(
            `    ${index + 1}. ${poem.title}${
              authors ? ` (by ${authors})` : ""
            }`
        ...
        infoEdition.images?.forEach((editionImage, index) => {
          ...
          const authors =
            image.authors?.map((ia) => ia.author.name).join(", ") || "";
          console.log(
            `    ${index + 1}. ${image.filename} (${editionImage.usageType})${
              authors ? ` by ${authors}` : ""
            }`
          );
        });
      ...
      default:
        ...
        console.log(
          "  generate <edition-id>      Generate PDF for specific edition by ID"
        );
        console.log(
          "  issue <issue-number>       Generate PDF for specific issue number"
        );
        console.log(
          "  date <date>                Generate PDF for specific date (e.g., 2025-09-22)"
        );
        console.log("  preview <issue-or-date>    Generate preview screenshot");
        console.log(
          "  info <edition-id>          Show detailed info about an edition"
        );
        console.log(
          "  test-url [url]             Test PDF generation with specific URL"
        );
        console.log(
          "  all                        Generate PDFs for all editions"
        );
        console.log("");
        console.log("Environment Variables:");
        console.log(
          "  BASE_URL                   Base URL for the print route (default: http://localhost:5173)"
        );
        console.log("");
        ...
        console.log(
          "  node scripts/generate-pdf-v2.js test-url http://localhost:5173/print?issue=1"
        );
        console.log("");
